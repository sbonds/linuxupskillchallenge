...
* Use `less` or `vi` to view the code of the default page - at `/var/www/html/index.html`, which is defined via *DocumentRoot* in the above `000-default.conf` file. This uses fairly complex modern web design - so you might like to browse to http://54.147.18.200/sample where you'll see a much simpler page. Use View Source in your browser to see the code of this, copy it, and then, at the linux command line, enter `sudo vim /var/www/html/index.html` to first delete the existing content, then paste in this simple example - and then edit to your own taste. View the result with your workstation browser by again going to _http://[external IP of your server]_
